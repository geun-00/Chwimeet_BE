...
public interface CategoryAdmApi {
    ...
    ResponseEntity<CategoryResBody> createCategory(@Valid @RequestBody CategoryCreateReqBody categoryCreateReqBody);

    @Operation(summary = "카테고리 수정 API", description = "카테고리 이름 수정, 수정된 카테고리와 함께 연관된 하위 카테고리들 응답")
    ResponseEntity<CategoryResBody> updateCategory(
    ...
    ResponseEntity<Void> deleteCategory(@PathVariable("id") Long categoryId);
}
