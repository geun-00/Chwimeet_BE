...
public class PostService {
    ...
    public void updatePost(Long postId, PostUpdateReqBody reqBody, long memberId) {
        Post post = this.postRepository.findById(postId).orElseThrow(() -> new ServiceException(HttpStatus.NOT_FOUND, "존재하지 않는 게시글입니다."));
        ...
        post.updatePost(reqBody.title(), reqBody.content(), reqBody.receiveMethod(), reqBody.returnMethod(), reqBody.returnAddress1(), reqBody.returnAddress2(), reqBody.deposit(), reqBody.fee());
        ...
        List<PostRegion> newPostRegions = this.regionRepository.findAllById(reqBody.regionIds()).stream().map(region -> new PostRegion(post, region)).toList();
