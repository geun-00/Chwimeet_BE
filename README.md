...
- 게시글의 후기 내용들을 토대로 게시글/사용자의 후기를 요약
- 사용자가 최대한 최신의 후기 정보를 참고할 수 있도록 최근 30개까지만 LLM에게 전달
- 응답시간 감소 및 LLM 호출 비용 감소를 위해 캐싱 처리
...
취밋 개발 초기에는 익숙한 관계형 데이터베이스인 MySQL을 사용했지만,
검색 정확도 향상과 임베딩 기반 분류 기능을 구현하기 위해 **벡터 검색(Vector Search)** 이 필요한 상황이 되었습니다.

외부 Vector Database(Chroma, Qdrant) 등도 검토했으나,
...
취밋에서 2-Stage RAG 구조를 적용하면서, 임베딩 단계와 Re-Rank 단계, 그리고 최종 답변 생성 단계를 각각 다른 모델로 분리해 사용하는 설계가 필요했습니다.  
Spring AI는 이러한 구조를 전제로 한 것처럼, 여러 AI 모델을 역할별로 유연하게 조합할 수 있도록 잘 추상화되어 있었습니다.

특히 2-stage RAG에서 핵심이 되는 **두 개 이상의 LLM을 동시에 사용하는 구조**를 설정 중심으로 구성할 수 있어, 모델을 교체하거나 추가하더라도 비즈니스 로직에 미치는 영향을 최소화할 수 있었습니다.  
Re-Ranker 모델과 답변 생성 모델을 분리해 사용하면서도, 코드 레벨에서는 일관된 방식으로 호출할 수 있었던 점이 큰 장점이었습니다.

또한 임베딩 과정에서 텍스트를 청크로 분할하고, 벡터로 변환한 뒤 Vector Store에 저장하는 흐름을  
모델과 설정만으로 처리할 수 있어, RAG 파이프라인 구현 자체에 집중할 수 있었습니다.  

무엇보다 Spring 기반 프로젝트였기 때문에 AI 기능을 기존 서비스 구조 안에 자연스럽게 녹일 수 있었고,  
추가 서버나 별도의 AI 전용 인프라 없이도 빠르게 기능을 확장할 수 있었습니다.  
...
Spring의 `@Scheduled`로도 가능했지만, 배치 작업이 중복 실행될 위험과
작업 이력 관리의 어려움 때문에 Quartz를 선택했습니다.

Quartz는 Job과 Trigger를 분리해 작업 단위별 스케줄 관리가 가능하고,  실행 흐름을 Listener 기반으로 추적할 수 있어 배치 작업의 상태를 명확하게 파악할 수 있었습니다.
...
서비스 운영 과정에서 사용자 요청과 분리하여
**정해진 시간에 자동으로 처리되어야 하는 작업**들을
Quartz 기반 배치 시스템으로 구현했습니다.

| 작업명 | 실행 주기    | 목적 |
|--------|----------|------|
| 게시글 임베딩 | 매시간 정각  | 사용자 응답 속도 개선 |
| 예약 상태 변경 | 매일 17:00  | 예약 프로세스 자동화 |
| 리마인드 알림 전송 | 매일 10:00 | 사용자 경험 개선 |
| 알림 정리 | 매일 17:00 | 데이터베이스 용량 관리 |
| 자동 제재 처리 | 매일 17:00 | 서비스 품질 유지 |
...
- **작업 단위 분리**  
각 배치 작업을 독립적으로 관리하여  
특적 작업의 실패가 다른 작업에 영향을 주지 않도록 설계

- **실행 흐름 가시성 확보**  
  배치 작업의 시작, 성공, 실패 여부를 로그로 명확히 확인 가능하도록 구성

- **비즈니스 로직 재사용성**  
Job은 실행 트리거 역할만 담당하고,  
실제 로직은 서비스 계층에 위임하여 API 로직과 재사용 가능하도록 설계
...
```
- 애플리케이션 시작 시 스케줄 자동 등록
- 기존 Job 존재 여부를 확인하여 **중복 등록** 방지
- Trigger 설정 변경 시 재등록 방식으로 일관성 유지
...
이를 통해 배치 작업의 상태를 쉽게 추적하고,  
문제 발생 시 원인을 빠르게 파악할 수 있도록 했습니다. 

<br>

#### 3. Quartz Job과 Spring 연계
Quartz Job 내부에서도 Spring Bean을 사용할 수 있도록
SpringBeanJobFactory를 확장한 커스텀 JobFactory를 구현했습니다.
```java
...
```
- Quartz Job에서도 Service, Repository Bean 주입 가능
- 기존 서비스 계층 로직을 그대로 재사용
...
사용자 질의에 대해 **벡터 검색으로 후보를 빠르게 수집하고, LLM 재순위화로 정확도를 높인 뒤**, 최종적으로 **질문 의도에 가장 부합하는 게시글만으로 자연스러운 답변을 생성**하는 2단계 검색 시스템입니다.

단순 텍스트 유사도가 아닌 **거래 조건(카테고리, 가격, 지역, 거래 방식)을 고려한 맥락 기반 검색**을 제공합니다.
...
- **검색 정확도 향상**
  - 벡터 유사도만으로 Top-K를 추출하면 실제로는 부적합한 게시글이 혼재
 > **예시**
>
> - **질문**: 문서 작업용 노트북을 대여하고 싶어
> - **검색 결과 (Top-3)**:
>   - 노트북 대여 게시글 A
>   - 노트북 대여 게시글 B
>   - 태블릿(패드) 대여 게시글 C ← 의도와 불일치
  - Re-Ranker로 질문 의도와 실제 관련 있는 게시글만 선별
- **응답 품질 개선**
  - 부적합한 게시글이 컨텍스트에 포함되면 LLM 답변이 부정확하거나 혼란스러워짐
  - 필터링된 게시글만 전달하여 일관되고 정확한 답변 생성
- **효율적인 처리**
  - 1차 벡터 검색으로 후보 범위를 빠르게 축소
  - 2차 LLM 검증으로 정밀 필터링
- **맥락 기반 판단**
  - 제목, 카테고리, 가격, 거래 방식, 지역 등 실제 거래 조건 반영
  - 질문 의도를 이해하고 적합한 게시글만 선별
...
사용자 질의
    ↓
1단계: Vector Search (후보 수집)
  - 코사인 유사도 기반 Top-5 조회
    ↓
2단계: Re-Rank (LLM 필터링)
  - GPT-4.1 mini로 후보 검증
  - 질문 의도와 실제 관련 있는 게시글만 선별
    ↓
3단계: Answer Generation (응답 생성)
  - GPT-5.1로 최종 답변 생성
    ↓
사용자에게 답변 반환
```
...
#### 1. Vector Search - 후보 게시글 빠른 수집
```java
List<Long> candidatePostIds = postVectorService.searchPostIds(query, 5);
```

- **MariaDB Vector**를 활용한 코사인 유사도 기반 검색
- **text-embedding-3-small** 임베딩 모델 사용
- Spring AI를 통해 사용자 질의를 벡터로 변환 후 유사도 계산
- Top-5 후보 게시글 ID를 빠르게 조회

---

#### 2. Re-Rank - LLM 기반 정밀 필터링
```java
...
- **GPT-4.1 mini**를 통한 후보 게시글 직접 검토
- 벡터 유사도가 높아도 질문 의도와 맞지 않는 게시글 제거
- **전달 컨텍스트**: 제목, 카테고리, 가격, 거래 방식, 지역 정보
- 단순 텍스트 유사도가 아닌 **실제 거래 조건 기준 판단**
- JSON 형식으로 적합한 게시글 ID 리스트 반환

---

#### 3. Answer Generation - 최종 응답 생성
```java
String prompt = answerPrompt.formatted(query, context);
return answerClient.prompt(prompt).call().content();
```

- Re-Rank를 통과한 게시글만으로 최종 응답 생성
- **GPT-5.1** 활용한 자연스러운 답변 생성
- 필터링된 게시글 정보를 컨텍스트로 제공
- 사용자 질문에 직접적으로 답하는 형태로 응답 구성

---

#### Re-Ranker 모델 추가로 필터링 품질 강화

1. **1차 벡터 검색**: 빠른 후보군 생성 (성능 확보)
2. **2차 LLM 검증**: 질문 의도 기반 정확한 게시글 선별 (정확도 확보)
   - 5개 후보 중 실제로 적합한 게시글만 선별
   - 부적합한 게시글은 제거하여 노이즈 차단
3. **3차 응답 생성**: 검증된 게시글만으로 답변 생성 (품질 확보)
...
이를 통해 단순 텍스트 매칭이 아닌 **실제 거래 상황을 고려한 검색**이 가능합니다.
</details>
